...
class FlightType(models.Model):
    FlightCategory = models.ForeignKey(FlightCat,on_delete=models.CASCADE,null=True)
...
class FlightBook(models.Model):
    ...
    def __str__(self):
        return f'{self.user} booked Filght Type = {self.Flight} from = {self.check_in.strftime("%d-%b-%Y %H:%M")} To = {self.check_out.strftime("%d-%b-%Y %H:%M")}'
